...
systems. The current package covers validation, quality control, cleaning,
track segmentation, movement, spatial use, resource use, social proximity,
mapping, and GPS-derived activity states.

The public API uses short `gps_*` function names. Future livestock sensor
modules may use the `acc_*`, `vf_*`, `rs_*`, and `fusion_*` prefixes when those
workflows are ready.
...
Additional study fields such as `animal_id`, `deployment_id`, `paddock`, and
`treatment` are retained where possible.
...
Functions return `data.frame` by default. Cleaning, regularisation, and model
functions retain useful diagnostics in documented attributes. The complete
development walkthrough is available in
[`dev/public-function-walkthrough.R`](dev/public-function-walkthrough.R).
